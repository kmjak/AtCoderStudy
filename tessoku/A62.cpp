 ...
 * 考察
 * DFSで解くという制約
 * 現在位置と訪問済みの頂点をDFSで管理する。
 * また、未訪問の場合は訪問済みにしてスコアを1増やす。
 * 最後にscoreとNを比較して同じであればグラフは連結であると判断する。
 */

vvll G(1e5+10);
ll score = 1;

void dfs(ll cur, vb &vis){
  for(ll nx: G[cur]){
    if(vis[nx])continue;
    vis[nx] = true;
    score++;
...
int main(){
  ...
  vis[0] = true;
  dfs(0, vis);
  cout<<(score==N?"The graph is connected.":"The graph is not connected.")<<endl;
  return 0;
}