  ...
  while(turn<T&&isContinue){
    ...
    rep(i,0,5000){
      ...
      if(resSize!=0){
        ...
        ll score=profit*max(0,(T-(turn+resSize+need)))+(1000*(2-createStationNum))-resSize*100;
    ...
    while(!ranking.empty()){
      ...
      ll need=(100*routeSize+(5000*createStationNum)-K)/turnToProfit+1;
      if((T-(turn+routeSize+need+createStationNum))*profit>=100*routeSize+(5000*createStationNum)){
