...
// 何回スキップするかを記録する
map<int,int> skipHouseStationCnt,skipJobStationCnt;
...
int main()
{
  ...
  map<int,int> apperHouseStationCnt;
  int maxFromHouse=-1;
  ...
  rep(i,0,50){
    rep(j,0,50){
      ...
      repa(house,howManyUsers){
        if(apperHouseStationCnt[house.second]<skipHouseStationCnt[house.second]){
          apperHouseStationCnt[house.second]++;
          continue;
        }
  ...
  int maxFromJob=-1;
  map<int,int> apperJobStationCnt;
  ...
  rep(i,0,50){
    rep(j,0,50){
      ...
      repa(job,howManyUsers){
        if(apperJobStationCnt[job.second]<skipJobStationCnt[job.second]){
          apperJobStationCnt[job.second]++;
          continue;
        }
  ...
  if(maxFromHouse==maxFromJob){
    // fromHouseからtoJobとfromJobからtoHouseの経路の長さがK以下の場合
    if(K>=100*resHouse.size() && K>=100*resJob.size()){
      // fromHouseからtoJobの経路の方が長い場合
      if(resHouse.size()>resJob.size()){
        G[fromHouseIdx.y][fromHouseIdx.x]=0;
        G[toJobIdx.y][toJobIdx.x]=0;
        skipHouseStationCnt[maxFromHouse]++;
        cout<<0<<fromHouseIdx<<endl;
        cout<<0<<toJobIdx<<endl;
        rep(i,0,resHouse.size()){
          G[resHouse[i].line.y][resHouse[i].line.x]=1;
          cout<<resHouse[i].direction<<resHouse[i].line<<endl;
        }
        K-=100*resHouse.size();
        T-=(resHouse.size()+2);
      }
      // fromJobからtoHouseの経路の方が長い場合
      else{
        G[fromJobIdx.y][fromJobIdx.x]=0;
        G[toHouseIdx.y][toHouseIdx.x]=0;
        skipJobStationCnt[maxFromJob]++;
        cout<<0<<toHouseIdx<<endl;
        cout<<0<<fromJobIdx<<endl;
        rep(i,0,resJob.size()){
          G[resJob[i].line.y][resJob[i].line.x]=1;
          cout<<resJob[i].direction<<resJob[i].line<<endl;
        }
        K-=100*resJob.size();
        T-=(resJob.size()+2);
      }
    }else if(K>=100*resHouse.size()){
      G[fromHouseIdx.y][fromHouseIdx.x]=0;
      G[toJobIdx.y][toJobIdx.x]=0;
      skipHouseStationCnt[maxFromHouse]++;
      cout<<0<<fromHouseIdx<<endl;
  ...
  // maxFromHouseの方が大きい場合
  else if(maxFromHouse>maxFromJob){
    if(K>=100*resHouse.size()){
      G[fromHouseIdx.y][fromHouseIdx.x]=0;
      G[toJobIdx.y][toJobIdx.x]=0;
      skipHouseStationCnt[maxFromHouse]++;
      cout<<0<<fromHouseIdx<<endl;
