...
int main()
{
  ...
  rep(i,0,50){
    rep(j,0,50){
      ...
      repa(to,GridStations[i][j]){
        ll dis = calcMoney(from,to.first);
        if(dis*100+10000>K)continue;
        vector<LINE> res=bfs(from,to.first);
        PNT fStation=from;
        PNT tStation=to.first;
        set<ll> idx=to.second.idx;
        ll p=to.second.money;
        if(res==vector<LINE>{}){
          continue;
        }
        ll resSize=res.size();
        int t=turn+resSize+2;
        double cost=100*resSize+10000;
        ll maxProfit=p*(T-t);
        ll score=maxProfit-cost;
        if(cost>K)continue;
        if(maxProfit<cost)continue;
        if(score>maxScore){
          maxScore=score;
          fromStation=from;
          toStation=to.first;
          idx=to.second.idx;
          profit=to.second.money;
          route=res;
        }
      }
    }
  ...
  if(maxScore!=-1){
    ...
    G[fromStation.y][fromStation.x]=0;
    G[toStation.y][toStation.x]=0;
    repa(l,route){
      cout<<l.direction<<l.line<<endl;
      G[l.line.y][l.line.x]=1;
    }
    turn+=route.size()+2;
    K-=(100*route.size()+10000);
    stations.eb(STATION{profit,turn});
  }
  rep(i,turn,T/2){
    cout<<-1<<endl;
  }
