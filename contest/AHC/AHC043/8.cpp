  ...
  rep(k,1,8){
    ...
    if(k*100<turn){
      ...
      repa(station,stations){
        K+=station.profit*max(0,(100*k-turn));
      }
      chmax(turn,100*k);
      continue;
    }
    ...
    rep(i,0,50){
      rep(j,0,50){
        ...
        repa(to,GridStations[i][j]){
          ...
          ll score=to.second.money*max(0,(T-turn))-d*100;
          scoreRanking.push(mp(score, RANKING{from, to.first, to.second.money, d}));
        }
      }
      // cout<<endl;
    }
    ...
    while(!isDecide&&!scoreRanking.empty()){
      ...
      if(resSize!=0){
        if(K>=100*resSize+10000&&(T-turn)*profit>=100*resSize+10000){
          ...
          K+=profit;
          repa(station,stations){
            K+=station.profit*max(0,(turn-(100*(k-1))));
          }
