...
/**
 * 考察
 * Mが極端に大きいので累積和の応用で解いた
 * また、二分探索を使用することで計算量を削減
 */
...
int main(){
  ...
  vector<pair<ll,ll>> imos(K*2+1);
  ll i=0;
  for(auto [j,c]: cnt){
    imos[i+1]={imos[i].first+c, j};
    i++;
  }
  for(auto [j,c]: cnt){
    imos[i+1]={imos[i].first+c, j+M};
    i++;
  }
  ...
  while(idx<=K){
    auto [s,j]=imos[idx];
    auto [ns,nj]=*lower_bound(imos.begin()+idx, imos.end(),mp(C+s,0LL));
    res+=(ns-s)*((min(M,imos[idx+1].second)-j));
    idx++;
    // cout<<res<<" ("<<nj<<" "<<j<<") ("<<ns<<" "<<s<<")"<<endl;
  }
  cout<<res<<endl;
  return 0;
}
