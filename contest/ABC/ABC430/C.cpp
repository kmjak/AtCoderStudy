...
int main(){
  ...
  ll l=1;
  while(l<=N){
    ll ar=lower_bound(sum_a.begin()+l,sum_a.end(),sum_a[l-1]+A)-sum_a.begin();
    ll br=upper_bound(sum_b.begin()+l,sum_b.end(),sum_b[l-1]+B-1)-sum_b.begin()-1;
    // cout<<l<<" "<<ar<<" "<<br<<endl;
    res+=max(0LL,br-ar+1);
    l++;
  }
