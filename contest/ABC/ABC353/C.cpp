...
/**
 * 考察
 * 1<=i<j<=Nの組み合わせのうち、A[i]+A[j]が1e8以上になるものの個数を数える
 * 言い換えるとi+1以上の中からA[j]が1e8-A[i]以上になるjの個数を数えればいい
 * なので、Aをソートして、1<=i<j<=Nの条件を満たすような値を2分探索で数える
 * 次に、その数*1e8を引く
 * 最後に各数値にN-1をかけた数を足す
 *
 * ここで先に数を足してしまうとlong longのオーバーフローが起きるので注意(これのせいでWAになった)
 */
...
int main(){
  ...
  ll o=0;
  for(ll i=0; i<N; i++){
    o+=N-(lower_bound(A.begin()+(i+1), A.end(), 1e8-A[i])-A.begin());
    // cout<<o<<endl;
  }
  ll res=-1e8*o;
  for(ll i=0;i<N;i++){
