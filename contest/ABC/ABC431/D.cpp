...
int main(){
  ...
  for(ll i=0;i<N;i++){
    vll nx(off*2+1,-inf);
    for(ll w=-sum;w<=sum;w++){
      ll j=w+off;
      if(dp[j]==-inf)continue;
      if(-sum<=w-W[i]&&w-W[i]<=sum)chmax(nx[w-W[i]+off],dp[j]+H[i]);
      if(-sum<=w+W[i]&&w+W[i]<=sum)chmax(nx[w+W[i]+off],dp[j]+B[i]);
    }
    dp.swap(nx);
  }
  ll res=0;
  for(ll w=0;w<=sum;w++)chmax(res,dp[w+off]);
  cout<<res<<endl;
  return 0;
}