 ...
 * 考察
 * むずい
 * 答えはmax(L) or max(L)+min(L)の2パターンしかないので
 * それに気づいたら解くだけ
 */
...
int main(){
  ...
  vector<pair<ll,ll>> m;
  for(ll l: L){
    if(m.empty()||m.back().first!=l)m.eb(l,1LL);
    else m.back().second++;
  }
  ...
  while(r-l>=0){
    if(r==l){
      if(m[r].second%2==1)ok=false;
    }else{
      if(m[l].first+m[r].first!=tg)ok=false;
      if(m[l].second!=m[r].second)ok=false;
    }
    r--;l++;
  }
