...
int main()
{
  ...
  rep(i,0,Q){
    ...
    if(opt==1){
      ...
      swt[i].swt1 = nest1;
      swt[i].swt2 = nest2;
      nestLastUpdated[nest1].eb(i);
      nestLastUpdated[nest2].eb(i);
    }else{
      int bd;
      cin>>bd;
      ...
      while(true){
        auto last = ubs(nestLastUpdated[nest],turn);
        if(last==nestLastUpdated[nest].end()){
          break;
        }
        int idx = *last;
        if(swt[idx].swt1==nest){
          nest = swt[idx].swt2;
        }else{
          nest = swt[idx].swt1;
        }
