 ...
 * 考察
 * 考え方自体は35分の提出であっていたが
 * 未達成の箇所の想定を忘れていて1WA + 15分で順位が爆下がり
 * Algoで初の水パフォをとれると期待してた。
 *
 * DPで各状態から横に遷移するか斜めに遷移するかでスコアを計算して出力
 */
...
int main(){
  ...
  for(ll i=1;i<N;i++){
    for(ll j=0;j<3;j++){
      ...
      chmax(dp[j][i],dp[j][i-1]+X[j][i]);
      if(j!=2)chmax(dp[j+1][i],dp[j][i-1]+X[j+1][i]);
    }
  }
  cout<<dp[2][N-1]<<endl;
  // for(auto x: dp){
